...
// 1. Zawiera listę aut
// 2. Implementuję metodę za pomocą której można dodać auto do listy
public class AutoController {
//    private Auto [] autos = new Auto[100];     -> deklaracje pustej tablicy aut o długości 100

    private ArrayList<Auto> autos = new ArrayList<>();

    public Auto addAuto(String brand, String model, Engine engine, Fuel fuel, double price) {
        // Utworzenie obiektu klasy auto
        Auto auto = new Auto(brand, model, engine, fuel, price);
        // zapis do listy aut
        autos.add(auto);
        return auto;
    }
    ...
    public void printAllAutos() {
        for (int i = 0; i < autos.size(); i++) {
            System.out.printf("| %3d | %15s | %15s | %15s | %15s | %15.2fzł |\n",
                    i + 1, autos.get(i).getBrand(), autos.get(i).getModel(), autos.get(i).getEngine(),
                    autos.get(i).getFuel(), autos.get(i).getPrice());
        }
    }
